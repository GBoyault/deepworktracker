...
export const ProjectSelect = (props: periodProps) => {
  ...
  const projectsList = props.projects.map((project) => (
    ...
    >
      ...
      <button
        ...
        onClick={(e) => {
          deleteProjectHandler(e, project.id);
        }}
      >
        &#10006;
      </button>
  ...
  return (
    <div
      ...
      style={
        props.selectedProject ? { color: props.selectedProject.color } : {}
      }
    >
      <div
        className={`${classes.selection} ${!props.selectedProject ? classes["selection--empty"] : ""}`}
        role="button"
      >
        {selection}
